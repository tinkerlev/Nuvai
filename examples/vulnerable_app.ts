...
// Description:
// This TypeScript file simulates a government appointment scheduler frontend using common service logic.
// Despite appearing legitimate, it contains vulnerabilities that often go unnoticed in AI-generated
// or no-code environments. This file is used to validate Nuvai’s TypeScript scanner.
...
interface Appointment {
  ...
  async function loadUserProfile(userId: string): Promise<void> {
    const response = await fetch("http://gov.example.com/profile/" + userId); // HIGH: unencrypted
    const user = await response.json();
    console.log("Loaded profile for:", user); // DEBUG info leaked
  }
