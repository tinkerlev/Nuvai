...
# ───────────────────────────────────────────────────────────────
# Nuvai PRO MAX Starter – Interactive Mode 🚀
#
# 🚀 Purpose:
# Launch Backend (Flask) and/or Frontend (React) intelligently based on user input.
#
# ✅ Features:
# - Smart OS detection
# - Check essential files and ports
# - Launch backend, frontend, or both
# - Auto-launch browser when running both
#
# ───────────────────────────────────────────────────────────────
...
open_browser() {
    ...
    if [[ "$OS_TYPE" == "macos" ]]; then
        ...
        xdg-open http://localhost:5173/ &> /dev/null || echo "⚠️ Could not auto-open browser."
    fi
}
